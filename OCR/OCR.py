...
def parse_arg():
    ...
    parser.add_argument('--image_path', type=str, default='OCR/images/for_ocr.png', help='the path to your image')
...
def recognition(config, img, model, converter, device):
    ...
    model.eval()
    preds = model(img)
    preds1 = preds.squeeze()                      # get shape of [M,6736] log_softmax matrix,M is the possible word
    #print(preds.shape)
    _, preds = preds.max(2)
    preds = preds.transpose(1, 0).contiguous().view(-1)    #become 1-dim
